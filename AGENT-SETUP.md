...
4. Help me connect my bot:
   ...
   - After the Codex hook is trusted, trigger a harmless permission request and
     ...
     green update on the original message without a second receipt message,
     and the command runs. Also confirm that replying with the plain word
     `allow` works. Keep the underlying Codex wait alive until the answer
     arrives; do not terminate its background cell while testing.
